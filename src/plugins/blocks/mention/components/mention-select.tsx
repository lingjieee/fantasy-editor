...
const Portal: ({ children: any }) => React.ReactPortal = ({ children }) =>
  ReactDOM.createPortal(children, document.body);

const MentionSelect: FunctionComponent<MentionSelectProps> = ({ at, options, valueIndex }) => {
    ...
    <Portal>
      <div ref={ref} className="fc-select-portal">
        {options.map((option, i) => (
          <div key={`${i}${option.value}`} className={classNames('fc-select-item', { selected: i === valueIndex })}>
            {option.value}
          </div>
        ))}
      </div>
